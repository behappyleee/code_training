...
//		전체 테스트 케이스에 주어지는 p의 길이의 합과 n의 합은 70만을 넘지 않는다.
		...
		for(int i=0; i<countCase; i++) {
			String[] functionStr = br.readLine().split("");
			int arrCount = Integer.parseInt(br.readLine());
			List<String> arrayList = new ArrayList<>(Arrays.asList(br.readLine().replaceAll("\\[|\\]", "").split(",")));
			...
			for(int j=0; j<functionStr.length; j++) {
				if(functionStr[j].equals("R")) {
					Collections.reverse(arrayList);
					continue;
				}
				if(functionStr[j].equals("D")) {
					if(arrayList.size() == 0) {
						errorMsg = "error";
						break;
					}
